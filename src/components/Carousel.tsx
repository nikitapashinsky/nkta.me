...
import type { PortfolioItem as PortfolioItemType, Product } from '~/data/portfolio';
import { PortfolioItem } from '~/components/PortfolioItem';
import { cn } from '~/utils/cn';

type CarouselProps = {
	product: Product;
	items: Array<[string, PortfolioItemType]>;
		...
		<div data-embla {...props}>
			<div
				className={cn(
					...
					// 'flex w-full flex-col pb-6 pl-4',
					// 'md:w-[90%] md:flex-row md:items-end md:justify-between md:pl-6',
				)}
			>
				<div className={'flex max-w-sm flex-col gap-2 text-balance'}>
					<div className={'flex items-baseline gap-2'}>
						<h3 className={cn('font-[550] tracking-tight')}>{product.name}</h3>
						<h3 className={cn('font-[550] tracking-tight text-neutral-400')}>{product.company}</h3>
					</div>
					<p className={cn('font-[450] tracking-tight text-neutral-600')}>{product.description}</p>
				</div>
